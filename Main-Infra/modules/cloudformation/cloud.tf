resource "aws_cloudformation_stack" "network" {
  ...
  parameters = {
    BastionSecurityGroupID = 	var.bastion_sg_id
    ClusterReplicaSetCount	= 3	
    Iops	= 100	
    KeyPairName	= var.KeyPairName
    MongoDBAdminPassword	= var.MongoDBAdminPassword
    MongoDBAdminUsername	= var.MongoDBAdminUsername
    MongoDBVersion	= 4.2
    NodeInstanceType	= var.NodeInstanceType
    PrimaryNodeSubnet	= var.PrimaryNodeSubnet
    Secondary0NodeSubnet	= var.Secondary0NodeSubnet
    Secondary1NodeSubnet	= var.Secondary1NodeSubnet
    QSS3BucketName	= "aws-quickstart"
    QSS3BucketRegion	= var.QSS3BucketRegion
    QSS3KeyPrefix	= "quickstart-mongodb/"
    ReplicaShardIndex = 0
    VPC	= var.vpc_id
